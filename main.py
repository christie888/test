...
def main():
    ...
    for i, movie in enumerate(files_list):
        ...
        if cap.isOpened()==False:
            ...
            for j in range(int(part_ns_movie.iat[0,5])):  #参照情報のグループ回数分繰り返す
                # それぞれグループごとに切り分ける
                ...
                # r(10), degree(11), color(12), LF(13)
                # ---------------------------------------------------------
                delta_r_grouping = 100
                for k in range(int(cur_obj_movie.iat[0,5])):  #取得情報のグループ回数分繰り返す
                    print("参照情報のグループと取得情報のグループの一致-----------")
                    print("参照情報グループ番号：", j)
                    print("取得情報グループ番号：", k)
                    cur_obj_group = cur_obj_movie.query('group_num == {}'.format(k))
                    x = int(cur_obj_group.iat[0,8])
                    y = int(cur_obj_group.iat[0,9])
                    X = int(part_ns_group.iat[0,8])
                    Y = int(part_ns_group.iat[0,9])
                    r, d = getxy_RD(x, y, X, Y)
                    if abs(r) >= delta_r_grouping:  #abs...絶対値  直線距離で閾値を超えた距離離れていたらそのグループではない
                        print("一致しませんでした.")
                        if k == int(cur_obj_movie.iat[0,5]) -1:
                            print("参照情報{}番グループに該当するグループを検知できなかったためスキップします.".format(j))
                            ("-------------------------------------------------------")
                            # cur_lamp_group に part_ns_group をいれる. 必要情報は None か ダミー文字列.
                            part_ns_group = part_ns_group.values.tolist()
                            cur_lamp_group = copy.deepcopy(part_ns_group)
                            for each_group in cur_lamp_group:
                                each_group[8], each_group[9] = "-", "-"
                                each_group[10], each_group[11] = "-", "-"
                                each_group[12], each_group[13] = "-", "-"
                                cur_lamp_movie.append(each_group)
                        print("-------------------------------------------------------")

                    else:  #グループの一致を確認
                        print("一致しました. グループごとの処理に移ります.")
                        print("-------------------------------------------------------")
                        ...
                        cur_lamp_group[0][8], cur_lamp_group[0][9] =  cur_obj_group[0][8], cur_obj_group[0][9] #取得オブジェクトの0番を無条件で0番ランプ認定（強引）
                        ...
                        delta_r = param["main"]["delta_r"]   #rの許容範囲
                        delta_d = param["main"]["delta_d"]   #degreemの許容範囲
