...
def main():
    ...
    for i, movie in enumerate(files_list):
        ...
        if cap.isOpened()==False:
            ...
            cur_lamp_movie = []   # cur_obj_movie を検証していき、確定させたランプ情報をここに入れていく
            ...
            if int(cur_obj_movie.iat[0,5]) != int(part_ns_movie.iat[0,5]):
                ...
                #gif画像生成
                # cur_lamp_movie をDF化
                _part_ns_movie = pd.DataFrame(part_ns_movie, columns = [
                    "ruck_num", "which_side", "shoot_position", "time_log", 
                    "group_num", "num_of_groups", "lamp_num", "num_of_lamps", "x", "y", 
                    "r", "degree", "color", "LF"
                ])
                module.make_gif.make_gif(undistort_frames, _part_ns_movie, movie_info, param, "mask_gif_cur")
                continue  #次のmovieに移る
            else:
                print("--------------------------------------------------------------------------------")
                print("参照情報のグループ数：", part_ns_movie.iat[0,5])
